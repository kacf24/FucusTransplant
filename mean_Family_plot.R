...
ggplot(test_day4_samples, aes(x=test_day4_samples$Family,y=test_day4_samples$avg)) + 
  geom_point(aes(shape=test_day4_samples$Treatment,color=test_day4_samples$Treatment),size=3)+coord_flip()+facet_grid(col=vars(test_day4_samples$Tissue))+xlab("Bacterial Family")+ylab("Mean Reads")+labs(shape="Treatment")+scale_shape_manual(values=c(15,16,17,3,4,16,15))+scale_color_manual(name="Treatment",values=c("red","green","blue","black","darkorange4","purple","tan4"))+theme_bw()+ theme(strip.text.x = element_text(size = 16,face="bold"),axis.text.y = element_text(size=8,face="bold"),axis.text.x = element_text(size=8,face="bold",angle=-90),axis.title.x = element_text(size=16, face="bold"),axis.title.y = element_text(size=16, face="bold"),legend.text=element_text(size=10,face="bold"),legend.title = element_text(size = 12,face="bold"))+scale_y_continuous(breaks = c(0,2500, 5000, 7500, 10000), labels = c("0","", "5000", "","10000"))
dev.off()
